...
export default class TabControl extends React.Component {
  constructor(props) {
    ...
    this.props["tab-item"].map(tabItem =>
      this.state.tabItems.push(tabItem.props.text)
    );
  ...
  getData() {
    ...
    const responsePromise = fetch(fetchStr, {
  ...
  handleClick() {
    ...
    this.getData().then(data => {
      this.state.tabItems = [];
      data.tabItems.forEach(child => this.state.tabItems.push(child.title));
      this.setState({
  ...
  render() {
    return (
      <React.Fragment>
        <h2 className="test-title">{this.props.title}</h2>
